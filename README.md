...
- 🩺 **会话健康卡片** —— 每个会话在话题顶部维护一张实时状态卡：进行中 / 等待确认 / 已完成，附模型、时长、消息数、Token 用量，每分钟自动刷新
...
- **会话健康卡片** —— 每个会话的话题顶部维护一张实时状态卡，含状态、模型、时长、消息数、Token 用量（按模型分组），每分钟刷新，会话结束后冻结
- **远程启动** —— 在聊天中发条消息，就能远程启动一个新的 Coding Agent
- **会话恢复** —— 话题中的 tmux 会话过期后，回复任意消息自动恢复
- **中途回执** —— Agent 还在忙时你发的消息，会先收到一个排队回执，等它空了再注入
...
> 想用会话健康卡片的标题自动精炼（可选），安装时带上 `summary` extra：
...
| `WALKCODE_EXTRA_ARGS` | 在 Agent 命令后插入额外参数 | Claude 走 Vertex：`--settings ~/.walkcode/vertex.json` |
...
每个会话的话题顶部维护一张交互卡片，由后台 poller 每分钟刷新，让你一眼看清这个会话的状态：
...
话题标题会自动用任务摘要命名。会话结束后卡片冻结，不再刷新。整个功能默认开启，`WALKCODE_HEALTH_CARD=0` 可关闭。

### 安全设计：远程启动的权限控制

通过聊天远程启动 Coding Agent 时，WalkCode 使用受控的权限模式启动 Agent（Claude Code 默认 `--permission-mode default`，Codex CLI 默认 `--ask-for-approval untrusted`），并通过 Hook 实现飞书端的权限审批：
...
| 中途追加 | Agent 忙时发消息 | 先收到排队回执，Agent 空了自动注入 |
| 会话过期 | 在旧话题中回复 | 自动恢复会话 |
...
| `WALKCODE_STATE_PATH` | 否 | 自定义状态文件路径（默认 `~/.walkcode/state.json`） |
...
| `WALKCODE_SUMMARY_VERTEX_PROJECT` | 否 | 健康卡片标题精炼用的 Vertex 项目（不设则用首行作标题） |
...
> 标题精炼（`WALKCODE_SUMMARY_*`）依赖可选的 `summary` extra（`anthropic[vertex]`）。未安装或未配置时自动降级为用任务首行作标题，不影响其他功能。
