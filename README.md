...
每个会话的话题顶部维护一张交互卡片。卡片在远程回复、本地输入、权限审批、Stop 结果等事件发生时立即刷新，让你一眼看清这个会话的状态：
