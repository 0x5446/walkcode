...
> 想用会话健康卡片的标题自动精炼（可选，仅 Codex 会话；Claude 用自带 AI 标题），安装时带上 `summary` extra：
...
话题标题会自动用任务摘要命名（Claude 用自带 AI 标题；Codex 可选用 Haiku 精炼，见配置项）。会话结束后卡片冻结，不再刷新。整个功能默认开启，`WALKCODE_HEALTH_CARD=0` 可关闭。
...
> 标题精炼（`WALKCODE_SUMMARY_*`）只用于 **Codex 会话**（Claude 用自带 AI 标题，不走这条路径），依赖可选的 `summary` extra（`anthropic[vertex]`）。未安装或未配置时 Codex 自动降级为用任务首行作标题，不影响其他功能。service account JSON 等凭证放仓库外、用绝对路径、不要提交进版本库。
