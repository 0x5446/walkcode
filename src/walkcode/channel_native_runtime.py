...
def _export_driver_label(environ=None) -> str:
    """ADR 0058 自杀陷阱守卫的标记面。

    serve 进程把自己的 launchd label（com.walkcode.<env 文件名去 .env>）写进
    自身环境；worker 生成时整体继承 os.environ，于是会话内的每个子进程（包括
    Bash 工具里跑的 upgrade.sh）都能据此识别"驱动我的 runtime 是谁"，从而
    跳过对它的立即重启。已有值时不覆盖——操作者显式指定优先。
    """
