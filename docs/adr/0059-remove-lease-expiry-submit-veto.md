...
- drain 恢复被延迟跨越 30s 守卫的窄窗（yield 挂起、事件循环被同步
  ...
  打断心跳与其他会话），接受；观测日志留痕。
- drain 视图滞后的权限浮出归因窄窗（终验第四轮）：排队 turn 已在
  CLI 侧启动并通过控制通道请求权限，而上一回合的 result 还躺在排水
  队列里未消费——该浮出会被归给仍"开着"的上一回合，随后被其核销
  重置；若该 turn 的流式开场消息因 worker 猝死从未到达，且此后静默
  满 30s，仍可能误判吸收。需要亚秒级对齐 + 开场消息丢失 + worker
  死亡 + 30s 静默四重叠加，且 CLI 侧控制请求先于流消息本身罕见；
  无可观测信号可区分（同 drain 滞后族），接受并留观测日志。
